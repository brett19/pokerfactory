...
function PokerPlayer(name, balance)
{
  this.name = name;
...
PokerTable.prototype.seatCount = function() {
...
PokerTable.prototype.playerInfo = function(myIdx, idx) {
  var player = this.seats[idx];
  if (!player) {
    return null;
  }
...
PokerTable.prototype.playerInfos = function(myIdx) {
  var playerInfos = [];
  for (var i = 0; i < this.seats.length; ++i) {
    playerInfos.push(this.playerInfo(myIdx, i));
  }
  return playerInfos;
};
...
PokerTable.prototype.info = function(myIdx) {
  var myOptions = null;
  if (myIdx !== -1) {
    myOptions = this.actionOpts(myIdx);
  }
