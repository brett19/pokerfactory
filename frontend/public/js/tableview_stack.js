...
function TableView_Stack(table, amount) {
  this.el = $('<div />');
  this.el.appendTo(table.el.find('.chipStacks'));
  ...
  for (var i = 0; i < stacks.length; ++i) {
    ...
    for (var j = 0; j < stack.length; ++j) {
      ...
      chipHtml += '<div class="chip chip' + num + '"></div>';
    }
    posX += CHIP_WIDTH;
  }
...
TableView_Stack.prototype.remove = function() {
  this.el.remove();
};

TableView_Stack.prototype.setPosition = function(x, y) {
  this.position.x = x;
  this.position.y = y;
...
TableView_Stack.prototype.moveTo = function(x, y, duration, handler) {
  this.position.x = x;
  this.position.y = y;
